...
public class Parser {
    ...
    public void getSymbol() {
        ...
        while (!isLast()) {
            ...
            int newChar = ' ';
            while (Character.isWhitespace(newChar) && !isLast())
                newChar = getChar();
            if (Character.isLetter(newChar) || newChar == '_') {
                ...
                if (CategoryCode.isReservedWord(token.toString())) {
                    tokenArrayList.add(new Token(token.toString(),
                            CategoryCode.name2code.get(token.toString()),
                            curLine));
                } else {
                    tokenArrayList.add(new Token(token.toString(),
                            "IDENFR",
                ...
                if (newChar == '/') {
                    ...
                    backward();
                    tokenArrayList.add(new Token("/",
                            CategoryCode.name2code.get("/"),
                            curLine));
                }
            } else if (newChar == '&') {
                token.append((char) newChar);
